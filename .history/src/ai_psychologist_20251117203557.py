...
class MemorySystem:
    ...
    def add_time_based_episodic_memory(self, time_ref: str, event_details: Dict[str, Any]):
        ...
        if existing_index is not None:
            ...
            # 确保existing有details字段
            if "details" not in existing:
                existing["details"] = []
            
            # 如果是旧的数据结构（包含interaction），转换为新的格式
            if "interaction" in existing and "details" in existing:
                # 将interaction转换为details格式
                interaction_data = existing["interaction"]
                converted_data = {
                    "user_message": interaction_data.get("user_message", ""),
                    "ai_response": interaction_data.get("ai_response", ""),
                    "emotional_insights": interaction_data.get("emotional_insights", {}),
                    "activity": self._extract_activity_from_interaction(interaction_data)
                }
                existing["details"].append(converted_data)
                # 移除旧的interaction字段
                del existing["interaction"]
            
            # 添加新的事件详情
            existing["details"].append(event_details)
            # 重新生成摘要
            existing["summary"] = self._summarize_time_events(existing["details"])
        else:
            # 添加新的时间点记录
            self.episodic_memory.append(event_entry)
    ...
    def _extract_activity_from_interaction(self, interaction: Dict[str, Any]) -> str:
        """从交互数据中提取活动信息"""
    ...
    def _summarize_time_events(self, events: List[Dict[str, Any]]) -> str:
        ...
        for event in events:
            if "emotion" in event:
                summary_parts.append(f"表达了{event['emotion']}")
            if "topic" in event:
                summary_parts.append(f"讨论了{event['topic']}")
    ...
    def get_episodic_memory_by_time(self, time_ref: str) -> Optional[Dict[str, Any]]:
        ...
        for memory in self.episodic_memory:
            if abs(memory["timestamp"] - timestamp) <= tolerance:
                return memory
...
class AIPsychologist:
    ...
    def _process_time_reference(self, user_message: str) -> Optional[str]:
        ...
        if time_ref:
            ...
            if episodic_memory:
                # 处理不同的数据结构，添加更完善的错误处理
                try:
                    if "details" in episodic_memory and isinstance(episodic_memory["details"], list):
                        # 新的时间参考记忆结构
                        summary = episodic_memory.get("summary", "发生了某些事件")
                    elif "interaction" in episodic_memory:
                        # 旧的情景记忆结构
                        summary = episodic_memory.get("summary", "发生了某些事件")
                    else:
                        # 默认情况或未知结构
